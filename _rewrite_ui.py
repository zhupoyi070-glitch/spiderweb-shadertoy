...
# 1) tbShow 精简为两参(标题+悬念行);提示的展开/收起改由两个按钮管理
old_tbshow = '''function tbShow(title, goal, prog){
  tb.style.display = "block";
  tbTitle.textContent = title;
  tbProg.textContent = prog || "";
  // 只在目标文本变化时收起提示(修复:每帧重置导致点击展开后立即被压回)
  if (tbGoal.dataset.goal !== goal){
    tbGoal.dataset.goal = goal;
    tbGoal.textContent = goal;
    tbGoal.style.display = "none";
    $("tbHint").textContent = "💡 查看通关提示";
  }
  $("tbHint").style.display = goal ? "inline-block" : "none";
}'''
new_tbshow = '''function tbShow(title, flavor){
  tb.style.display = "block";
  tbTitle.textContent = title;
  tbProg.textContent = flavor;                      // 可见行:悬念式形容,具体内容看提示按钮
}'''
assert old_tbshow in s, "tbShow not found"
s = s.replace(old_tbshow, new_tbshow)
